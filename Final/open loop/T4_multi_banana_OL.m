...
coord_fork = [0,5.65,18];
coord_knife_elevate = coord_knife + [0,4,0];
coord_fork_elevate_start = coord_fork + [0,2,-10]
coord_fork_elevate_end = coord_fork + [0,4,0]

coord_slice1 = [15.5,4,-1.2];
coord_slice1_elevate = coord_slice1 + [0,8,0];
coord_slice2 = [15.5,4, 1.2];
coord_slice2_elevate = coord_slice2 + [0,8,0];
coord_slice3 = [15.5,4, 3.6];
coord_slice3_elevate = coord_slice3 + [0,8,0];
coord_slice4 = [15.2,4, -3.4];
coord_slice4_elevate = coord_slice4 + [0,8,0];
feed = 0;

coord_pick1 = [17.9,3,0];
coord_pick1_bend = coord_pick1 + [0,3,0];
coord_pick1_elevate = coord_pick1 + [0,10,0];

coord_pick2 = [17.9,3,2.6];
coord_pick2_bend = coord_pick2 + [0,3,0];
coord_pick2_elevate = coord_pick2 + [0,10,0];

coord_pick3 = [17.9,3,-2.6];
coord_pick3_bend = coord_pick3 + [0,3,0];
coord_pick3_elevate = coord_pick3 + [0,10,0];
...
while done == 0
    ...
    switch state
        ...
        case 'return knife'
            ...
            disp('1');
            %%elevate
            move(coord_knife_elevate, pose_knife2 , gripper_close);
            %pickup coord
            disp('2');
            move(coord_knife, pose_knife2, gripper_close);
            %pickup coord close gripper
            x = gripper_open;
            disp('3');
            move(coord_knife, pose_knife2, gripper_open2);
            %elevate
            disp('4');
            move(coord_knife_elevate, pose_knife2 , gripper_open2);
            disp('5');
            %reset position
            disp('6');
        ...
        case 'pick food'
            ...
            if feed_count == 2
                move(coord_pick2_elevate, pose_fork, gripper_close);
                move(coord_pick2, pose_fork, gripper_close);
                pause(1);
                angle2 = -45;
                move(coord_pick2_bend,-30 , gripper_close);
                move(coord_pick2_elevate, -30, gripper_close);
                state = 'feed'
            end

            if feed_count == 3
                move(coord_pick3_elevate, pose_fork, gripper_close);
                move(coord_pick3, pose_fork, gripper_close);
                pause(1);
                angle2 = -45;
                move(coord_pick3_bend,-30 , gripper_close);
                move(coord_pick3_elevate, -30, gripper_close);
                state = 'feed'
            end
