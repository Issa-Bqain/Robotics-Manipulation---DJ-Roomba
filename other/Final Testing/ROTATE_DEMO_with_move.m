...
% existing code
cube1_start = [17.5,3.9,17.5];
coord_cube1 = [0,3.9,15];
coord_cube2 = [17.5,4.2,-5];
coord_cube3 = [0,4.15,-17.5];
coord_reset = [27.4,20.5,0];
%elevations
cube1_start_elevate=cube1_start+[0,2,0];
...
coord_extend1=coord_rotate1 + [0,6,-7.5];
coord_rotate_elevate1 = coord_extend1 + [0,4,7.5];

coord_rotate2 = coord_cube2;
coord_extend2=coord_rotate2 + [-8.75,6,2.5];
coord_rotate_elevate2 = coord_extend2 + [8.75,4,-2.5];

coord_rotate3 = coord_cube3;
coord_extend3=coord_rotate3 + [0,6,8.75];
coord_rotate_elevate3 = coord_extend3 + [0,4,-8.75];
goalcoord = 0;
goalpose = 0;
gripper_pos = 0;
goalcoord_elevate=0;
...
while done == 0
    ...
    switch state
        ...
        case 'pick n grab'
            disp('second state')
            if cube_num == 1 
                startcoord=cube1_start;
                goalcoord = coord_cube1;
                startcoord_elevate=cube1_start_elevate;
                goalcoord_elevate = coord_cube1_elevate;
            
                move(startcoord_elevate,-45,gripper_open);
                move(startcoord,-45,gripper_open);
                move(startcoord,-45,gripper_close);
                move(startcoord_elevate,-45,gripper_close);
                move(goalcoord_elevate,-45,gripper_close);
                move(goalcoord,-45,gripper_close);
                move(goalcoord,-45,gripper_open);
                move(goalcoord_elevate,-45,gripper_open);
                state = 'rotate'
        ...
        case 'rotate'
            ...
            for i = 1:rotation_num
                ...
                move(coord_extend,goalpose,gripper_pos)
                %move to elevated pos in pickup pose
                goalpose=pose_pickup;
                move(cubecoord, goalpose,gripper_pos);
                %move to cube pos from elevated pos
                gripper_pos = gripper_close;
                move(cubecoord, goalpose,gripper_pos);
                %grab cube
                move(goalcoord_elevate,goalpose,gripper_pos)
                %move to elevated pos
                goalcoord = coord_rotate_elevate;
                move(goalcoord, goalpose,gripper_pos);
                %move to pre rotation coord
                goalpose = pose_rotate;
                goalcoord = coord_extend;
                move(goalcoord, goalpose,gripper_pos);
                %rotate to extended pos
                goalcoord = goalcoord_elevate;
                move(goalcoord, goalpose,gripper_pos);
                %move to above cube holder
                move(coord_rotate, goalpose,gripper_pos);
                %move down to holder
                gripper_pos = gripper_open;
                move(coord_rotate, goalpose,gripper_pos);
                %let go
                ...
                goalpose=pose_pickup;
                move(coord_rotate_elevate,goalpose,gripper_pos);
...
%                 move(goalcoord, goalpose,gripper_pos);
        ...
        case 'place end'
            ...
            move(goalcoord_elevate,-45,gripper_open);
            move(goalcoord,-45,gripper_open);
            move(goaltcoord,-45,gripper_close);
            move(goalcoord_elevate,-45,gripper_close);
            move(startcoord_elevate,-45,gripper_close);
            move(startcoord,-45,gripper_close);
            move(startcoord,-45,gripper_open);
...
theta1_IK = -1.5+(theta1_IK *180/pi);
