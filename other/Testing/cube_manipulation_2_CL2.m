...
%end coordinates
coord_cube1_end = [15,10,5];
coord_cube2_end = [7,7,17];
coord_cube3_end = [14,7,14];
...
while (a==0 || b==0 || c==0 || d==0)
    ...
    delta1=theta1_IK-present_theta1
    delta2=theta2_IK-present_theta2
    delta3=theta3_IK-present_theta3
    delta4=theta4_IK-present_theta4
...
disp('%%%%%%%%%%%%%%%%')
    ...
    if(theta1_IK >= present_theta1)
        ...
        %     end
        if (abs(present_theta1) <= (abs(theta1_IK)+3) && abs(present_theta1) >= (abs(theta1_IK)-3) && (sign(present_theta1) == sign(theta1_IK+2) || sign(present_theta1) == sign(theta1_IK-2)))
            a = 1;
            disp('a1')
        else
            a = 0;
            present_theta1 = present_theta1 + abs(delta1)/count;
        end
    else
        % if(theta1_IK < present_theta1)
        %     if (present_theta1 > theta1_IK*maxerror || present_theta1 < theta1_IK*minerror)
    ...
    if(theta2_IK >= present_theta2)
        if (abs(present_theta2) <= (abs(theta2_IK)+3) && abs(present_theta3) >= (abs(theta2_IK)-3) && (sign(present_theta2) == sign(theta2_IK+2) || sign(present_theta2) == sign(theta2_IK-2)))
            b = 1;
            disp('b1')
        else
            b = 0;
            present_theta2 = present_theta2 + abs(delta2)/count;
        end
    else
        if (abs(present_theta2) <= (abs(theta2_IK)+3) && abs(present_theta2) >= (abs(theta2_IK)-3) && (sign(present_theta2) == sign(theta2_IK+2) || sign(present_theta2) == sign(theta2_IK-2)))
            b = 1;
            disp('b2')
        else
            b = 0;
    ...
    if(theta3_IK >= present_theta3)
        if (abs(present_theta3) <= (abs(theta3_IK)+3) && abs(present_theta3) >= (abs(theta3_IK)-3) && (sign(present_theta3) == sign(theta3_IK+2) || sign(present_theta3) == sign(theta3_IK-2)))
            c = 1;
            disp('c1')
        else
            c = 0;
            present_theta3 = present_theta3 + abs(delta3)/count;
        end
    else
        if (abs(present_theta3) <= (abs(theta3_IK)+3) && abs(present_theta3) >= (abs(theta3_IK)-3) && (sign(present_theta3) == sign(theta3_IK+2) || sign(present_theta3) == sign(theta3_IK-2)))
            c = 1;
    ...
    if(theta4_IK >= present_theta4)
        if (abs(present_theta4) <= (abs(theta4_IK)+3) && abs(present_theta4) >= (abs(theta4_IK)-3) && (sign(present_theta4) == sign(theta4_IK+2) || sign(present_theta4) == sign(theta4_IK-2)))
            d = 1;
            disp('d1')
        else
            d = 0;
            present_theta4 = present_theta4 + abs(delta4)/count
        end
    else
        if (abs(present_theta4) <= (abs(theta4_IK)+3) && abs(present_theta4) >= (abs(theta4_IK)-3) && (sign(present_theta4) == sign(theta4_IK+2) || sign(present_theta4) == sign(theta4_IK-2)))
            d = 1;
