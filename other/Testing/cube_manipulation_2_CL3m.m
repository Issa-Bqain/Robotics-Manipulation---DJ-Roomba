...
%velocity
write4ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID_1, 112, 0);
write4ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID_2, 112, 0);
write4ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID_3, 112, 0);
write4ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID_4, 112, 0);
write4ByteTxRx(port_num, PROTOCOL_VERSION, DXL_ID_5, 112, 0);
...
% existing code
coord_cube1 = [22.5,5,0];
coord_cube2 = [7,5,20];
coord_cube3 = [15,5.5,-15];
coord_reset = [27.4,20.5,0];
%elevations
coord_cube1_elevate = coord_cube1 + [0,0.8,0];
coord_cube2_elevate = coord_cube2 + [0,4,0];
coord_cube3_elevate = coord_cube3 + [0,3,0];
...
%elevations
coord_cube1_end_elevate = coord_cube1_end + [0,0.7,0];
coord_cube2_end_elevate = coord_cube2_end + [0,4,0];
coord_cube3_end_elevate = coord_cube3_end + [0,4,0];
...
while (a==0 || b==0 || c==0 || d==0)
    ...
    if(theta1_IK >= present_theta1)
        ...
        %     end
        if (abs(present_theta1) <= (abs(theta1_IK)+2) && abs(present_theta1) >= (abs(theta1_IK)-2) && (sign(present_theta1) == sign(theta1_IK+2) || sign(present_theta1) == sign(theta1_IK-2)))
            a = 1;
            disp('a1')
        else
            a = 0;
    ...
    if(theta2_IK >= present_theta2)
        if (abs(present_theta2) <= (abs(theta2_IK)+2) && abs(present_theta3) >= (abs(theta2_IK)-2) && (sign(present_theta2) == sign(theta2_IK+2) || sign(present_theta2) == sign(theta2_IK-2)))
            b = 1;
            disp('b1')
        else
            b = 0;
        ...
        if (abs(present_theta2) <= (abs(theta2_IK)+2) && abs(present_theta2) >= (abs(theta2_IK)-2) && (sign(present_theta2) == sign(theta2_IK+2) || sign(present_theta2) == sign(theta2_IK-2)))
            b = 1;
            disp('b2')
        else
            b = 0;
    ...
    if(theta3_IK >= present_theta3)
        if (abs(present_theta3) <= (abs(theta3_IK)+2) && abs(present_theta3) >= (abs(theta3_IK)-2) && (sign(present_theta3) == sign(theta3_IK+2) || sign(present_theta3) == sign(theta3_IK-2)))
            c = 1;
            disp('c1')
        else
            c = 0;
    ...
    if(theta4_IK >= present_theta4)
        if (abs(present_theta4) <= (abs(theta4_IK)+2) && abs(present_theta4) >= (abs(theta4_IK)-2) && (sign(present_theta4) == sign(theta4_IK+2) || sign(present_theta4) == sign(theta4_IK-2)))
            d = 1;
            disp('d1')
        else
            d = 0;
    ...
    encoder_theta0 = -(theta1*2048 )/ 180 +2048
    encoder_theta1 = -(theta2*2048 )/ 180 +2048
    encoder_theta2 = -(theta3*2048 )/ 180 +2048
    encoder_theta3 = -(theta4*2048 )/ 180 +2048
